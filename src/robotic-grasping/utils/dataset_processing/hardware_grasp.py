...
class GraspDetector:
    def __init__(self):
        self.grasp_point_640 = None  # Class variable to store grasp_point_640
        self.grasp_angle = None
    
    def detect_grasps(self, q_img, ang_img, width_img=None, no_grasps=1):
        """
        ...
        grasps = []
        #Checking for best grasp value must be done currently takes only last value
        #even in the previous code
        for grasp_point_array in local_max:
            ...
            self.grasp_point_640 = (grasp_point_224[1] + crop_offset_x, grasp_point_224[0] + crop_offset_y)  # Updating class variable
            print(f'grasp for 640x480: {self.grasp_point_640}')
            
            self.grasp_angle = ang_img[grasp_point_224]
            g = Grasp(grasp_point_224, self.grasp_angle)
